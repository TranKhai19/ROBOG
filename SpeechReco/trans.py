...
while True:
    ...
    try:
        ...
        print("Ban da noi (tieng Anh):", text_en)
        sentences.append(text_en)

        text_vi = translator.translate(text_en)
        print("Ban dich (tieng Viet):", text_vi)
        translations.append(text_vi)

        if "stop" in text_en.lower():
            break
...
filename = f"D:/AnhTuan/CODE/SpeechReco/save_text/TransEnToVN/speech_recognition_{timestamp}.txt"

with open(filename, "w", encoding="utf-8") as f:
    for en, vi in zip(sentences, translations):
        f.write(f"Tieng Anh: {en}\n")
        f.write(f"Tieng Viet: {vi}\n\n")
